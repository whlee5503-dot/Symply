import type { Translations } from './en'

export const ko: Translations = {
  nav: {
    home:     '홈',
    history:  '기록',
    cycle:    '사이클',
    insights: '인사이트',
    report:   '보고서',
    settings: '설정',
  },

  home: {
    greeting_morning: "좋은 아침이에요",
    greeting_afternoon: "안녕하세요",
    greeting_evening: "좋은 저녁이에요",
    subtitle: "오늘 몸 상태는 어떤가요?",
    checkin_title: "오늘의 체크인",
    pain: "통증",
    fatigue: "피로",
    mood: "기분",
    sleep: "수면",
    sleep_unit: "시간",
    activity: "활동량",
    activity_low: "낮음",
    activity_medium: "보통",
    activity_high: "높음",
    triggers: "트리거",
    medications: "복약",
    notes: "메모",
    notes_placeholder: "오늘 추가로 기록할 내용이 있나요?",
    save: "체크인 저장",
    saved: "저장되었습니다!",
    saving: "저장 중…",
    already_logged: "오늘은 이미 기록하셨어요",
    edit: "수정",
    no_triggers_btn: '오늘 트리거 없음 ✓',
    no_triggers_active: '✅ 오늘 트리거 없음',
    triggers_section: '오늘의 영향 요인',
    triggers_food_label: '🍽 음식/음료',
    triggers_lifestyle_label: '😓 생활습관',
    triggers_environment_label: '🌤 환경',
    triggers_tooltip: '만성질환 연구에서 염증 및 증상 악화와 가장 관련성이 높은 요인들입니다.',
    trigger_gluten: '글루텐',
    trigger_dairy: '유제품',
    trigger_sugar: '설탕',
    trigger_caffeine: '카페인',
    trigger_alcohol: '알코올',
    trigger_high_fodmap: '고포드맵 식사',
    trigger_high_glycemic: '고혈당지수 식사',
    trigger_stress: '스트레스',
    trigger_poor_sleep: '수면 부족',
    trigger_pressure_change:    '기압 변화',
    trigger_temperature_change: '기온 변화',
    trigger_sun_exposure:       '햇빛 노출',
    trigger_overexertion: '과활동',
    triggers_primary_hint: '★ 표시는 대표 질환과 근거가 가장 강한 요인입니다. 표시 여부와 관계없이 모든 요인은 기록에 동일하게 반영됩니다.',
    no_meds: "등록된 약이 없습니다. 설정에서 추가하세요.",
  },

  history: {
    title: '기록',
    subtitle: '증상 달력',
    days_logged_count: '{n}일 기록됨',
    days_logged: '기록일수',
    avg_pain: '평균 통증',
    avg_fatigue: '평균 피로',
    no_data: '아직 기록이 없습니다. 홈에서 체크인을 시작하세요.',
    no_checkin: '이 날의 체크인 기록이 없습니다.',
    activity_label: '활동량:',
    triggers_label: '트리거:',
    cal_good: '좋음',
    cal_okay: '보통',
    cal_rough: '나쁨',
    cal_bad: '매우 나쁨',
    cal_nodata: '기록 없음',
    weekdays: ['일', '월', '화', '수', '목', '금', '토'],
    next_insight: '💡 인사이트 탭에서 패턴을 확인해보세요 →',
    pain: '통증',
    fatigue: '피로',
    sleep: '수면',
    mood: '기분',
    activity: '활동량',
    triggers: '트리거',
    notes: '메모',
    no_triggers: '기록 없음',
  },

  insights: {
    title: '인사이트',
    subtitle: '패턴이 여기에 표시됩니다.',
    subtitle_days: '{n}일간의 데이터 기반',
    no_data_title: '데이터가 부족합니다',
    no_data_body: '최소 7일 기록 후 패턴을 확인할 수 있습니다.',
    avg_pain: '평균 통증',
    avg_fatigue: '평균 피로',
    avg_sleep: '평균 수면',
    good_days: '좋은 날',
    pattern_insights: '패턴 인사이트',
    based_on_data: '내 데이터 기반',
    ai_title: 'AI 패턴 분석',
    ai_subtitle: 'Claude AI가 증상 데이터를 분석해 개인화된 패턴을 찾아드립니다.',
    ai_pro_subtitle: 'Claude AI가 증상 기록을 분석해 "수면 6시간 미만 시 통증 40% 상승" 같은 개인화된 인사이트를 제공합니다.',
    ai_unlock: 'AI 분석 잠금 해제',
    ai_free_remaining: '이번 달 무료 분석 남은 횟수: {n}/5',
    ai_free_limit_reached: '이번 달 무료 분석 5회를 모두 사용했습니다. Pro로 업그레이드하면 무제한으로 이용할 수 있습니다.',
    ai_analyze: '✨ 내 패턴 분석하기',
    ai_analyzing: '🔄 분석 중...',
    ai_reanalyze: '🔄 다시 분석',
    ai_demo_badge: '🧪 데모 분석 — 크레딧 충전 시 개인화된 AI 인사이트 제공',
    ai_doctor: '🩺 의사에게 전달할 내용:',
    pro_badge: '✨ PRO 기능',
    chart_pain_fatigue: '통증 & 피로 — 최근 30일',
    chart_sleep: '수면 — 최근 30일',
    top_triggers: '주요 트리거',
    days: '일',
  },

  report: {
    title: '진료 준비 보고서',
    subtitle: '다음 진료를 위한 임상 요약 PDF를 생성하세요.',
    pro_banner_title: 'Pro 기능 — 진료 준비 보고서',
    pro_banner_body: '의사가 3분 안에 읽을 수 있는 임상 요약 PDF를 생성합니다.',
    upgrade: '업그레이드',
    period_label: '보고서 기간',
    period_7: '7일',
    period_14: '14일',
    period_30: '30일',
    period_60: '60일',
    period_90: '90일',
    preview_title: '📋 보고서 미리보기',
    no_data: '최근 {n}일간 데이터가 없습니다. 홈에서 기록을 시작하세요.',
    days_logged: '기록일수',
    avg_pain: '평균 통증',
    flare_days: '악화일수',
    top_triggers: '주요 트리거:',
    disclaimer: '⚠️ 이 보고서는 정보 제공 목적이며 의학적 진단이 아닙니다.',
    disclaimer_short: '이 보고서는 정보 제공 목적이며 의학적 진단이 아닙니다.',
    download: '📄 PDF 보고서 다운로드',
    unlock: '🔒 PDF 보고서 잠금 해제 — Pro',
    generating: '⏳ PDF 생성 중...',
    days: '일',
  },

  cycle: {
    title: '생리 주기',
    today: '오늘',
    period_today: '오늘 생리 중인가요?',
    yes: '예',
    no: '아니요',
    flow_label: '양',
    flow_light: '적음',
    flow_medium: '보통',
    flow_heavy: '많음',
    symptoms_label: '증상',
    notes_label: '메모',
    notes_placeholder: '오늘에 대한 메모를 입력하세요...',
    save_btn: '생리 데이터 저장',
    saved: '✓ 저장됨!',
    weekdays: ['일', '월', '화', '수', '목', '금', '토'],
    symptom_cramps: '생리통',
    symptom_bloating: '복부 팽만',
    symptom_headache: '두통',
    symptom_mood_swings: '기분 변화',
    symptom_breast_tenderness: '유방 압통',
    symptom_spotting: '소량 출혈',
    symptom_clots: '혈전',
    disclaimer: '생리 데이터는 안전하게 저장되며 공유되지 않습니다. 의료 기기가 아닙니다 — 의료 전문가와 상담하세요.',
  },

  settings: {
    title: '설정',
    appearance: '화면 설정',
    theme_label: '테마',
    theme_sub: '라이트, 다크, 또는 시스템 설정 따르기',
    theme_light: '☀️ 라이트',
    theme_dark: '🌙 다크',
    theme_auto: '💻 자동',
    language_label: '언어',
    language_sub: '앱 표시 언어',
    gender_title: '생물학적 성별',
    gender_sub: '생리 주기 추적 및 인사이트 개인화에 사용됩니다',
    gender_female: '♀ 여성',
    gender_male: '♂ 남성',
    gender_other: '⚧ 기타 / 밝히고 싶지 않음',
    gender_cycle_note: '여성 사용자 전원에게 생리 주기 추적 기능이 제공됩니다. 루푸스, 섬유근통, 류마티스, IBS 등 많은 만성질환이 호르몬 주기의 영향을 받습니다.',
    conditions_title: '나의 질환',
    conditions_sub: '해당하는 것을 모두 선택하세요. AI 인사이트 개인화에 사용됩니다.',
    primary_condition_title: '대표 질환',
    primary_condition_sub: '일상에서 가장 영향을 크게 받는 질환을 선택하세요. 체크인 시 어떤 요인을 우선 표시할지 결정하는 데 사용됩니다.',
    primary_condition_none: '위에서 하나를 선택하면 일상 체크인이 개인화됩니다.',
    medications_title: '복약 관리',
    medications_sub: '체크인할 때 복약 여부를 기록하세요.',
    no_meds_yet: '아직 등록된 약이 없습니다.',
    add_med: '+ 약 추가',
    med_name_placeholder: '약 이름',
    med_freq_daily: '매일',
    med_freq_asneeded: '필요 시',
    med_freq_weekly: '매주',
    med_add_btn: '추가',
    med_cancel_btn: '취소',
    subscription_title: '구독',
    plan_free: '무료 플랜',
    plan_pro: 'Pro 플랜',
    plan_pro_active: '✅ Pro 활성화됨',
    plan_pro_expires: '만료일',
    upgrade_btn: '✨ Pro로 업그레이드',
    account_title: '계정',
    guest_banner_title: '게스트로 둘러보는 중입니다',
    guest_banner_body: '기록은 지금도 저장되지만, 브라우저 데이터를 지우면 사라질 수 있습니다. 계정을 연결하면 지금까지의 기록이 그대로 유지되며 다른 기기에서도 볼 수 있습니다.',
    guest_link_google: 'Google로 계정 연결',
    guest_link_email_toggle: '이메일로 계정 연결',
    guest_link_email_label: '이메일',
    guest_link_password_label: '비밀번호 (6자 이상)',
    guest_link_submit: '계정 연결하기',
    guest_link_cancel: '취소',
    guest_link_error: '문제가 발생했습니다. 다시 시도해주세요.',
    guest_link_email_in_use: '이미 가입된 이메일입니다. 다른 이메일을 사용해주세요.',
    guest_link_weak_password: '비밀번호는 6자 이상이어야 합니다.',
    signed_in_as: '로그인 중',
    about_guide: '용어 설명',
    about_guide_val: '통증 척도·질환·트리거 용어 풀이',
    about_feedback: '피드백 보내기',
    about_feedback_val: '의견이나 개선 사항을 알려주세요',
    about_title: 'About',
    about_version: '버전',
    about_privacy: '개인정보',
    about_privacy_val: '광고 없음. 데이터 판매 없음.',
    about_evidence: '근거 기반',
    about_evidence_val: 'MPH 전문가 설계',
    about_disclaimer: 'Symply는 의료 기기가 아닙니다. 모든 의료적 결정은 전문 의료인과 상담하세요.',
    cycle_tab_enabled: '💜 사이클 탭이 활성화되었습니다. 호르몬 주기는 많은 만성질환에 영향을 줍니다.',
    notifications_title: '매일 리마인더',
    notifications_sub: '매일 증상을 기록할 수 있도록 알림을 보내드립니다.',
    notifications_enable: '알림 켜기',
    notifications_time: '알림 시간',
    notifications_permission_denied: '알림이 차단되어 있습니다. 브라우저 설정에서 허용해주세요.',
    notifications_not_supported: '이 브라우저는 알림을 지원하지 않습니다.',
    sign_out: '로그아웃',
    sign_out_confirm: '정말 로그아웃 하시겠습니까?',
    sign_out_guest_title: '게스트 계정입니다',
    sign_out_confirm_guest: '게스트 계정은 다시 로그인할 방법이 없습니다. 지금 로그아웃하면 지금까지 기록한 모든 데이터가 영구적으로 사라집니다. 위의 "계정 연결" 기능으로 먼저 계정을 연결하는 것을 권장합니다.',
    sign_out_yes_guest: '그래도 로그아웃 (기록 삭제됨)',
    sign_out_yes: '로그아웃',
    sign_out_cancel: '취소',
    condition_PCOS: 'PCOS (다낭성 난소 증후군)',
    condition_endometriosis: '자궁내막증',
    condition_fibromyalgia: '섬유근통',
    condition_lupus: '루푸스',
    condition_rheumatoid_arthritis: '류마티스 관절염',
    condition_crohns: '크론병',
    condition_ibs: '과민성 장 증후군 (IBS)',
    condition_chronic_fatigue: '만성 피로 증후군 (ME/CFS)',
    condition_other: '기타',
    display_name_label: '표시 이름 (선택)',
    save: '변경사항 저장',
    saving: '저장 중…',
    saved: '저장됨',
  },

  onboarding: {
    next: '다음 →',
    back: '← 이전',
    get_started: '기록 시작하기 →',
    conditions_none: '나중에 설정 →',
    slide_welcome_title: '당신의 증상은\n들려질 자격이 있습니다.',
    slide_welcome_body: 'Symply는 매일 30초만으로 증상을 기록하고, 패턴을 발견하고, 의사에게 명확한 근거를 제시할 수 있도록 도와줍니다.',
    slide_problem_title: '당신이 상상하는 게 아닙니다.',
    slide_problem_body: '자궁내막증이나 섬유근통 같은 질환은 진단까지 평균 7~12년이 걸립니다. 의료 가스라이팅은 실제로 존재하며 — 기록된 데이터가 그것을 바꿀 수 있습니다.',
    slide_how_title: 'Symply 사용법',
    slide_conditions_title: '어떤 질환을 관리하고 있나요?',
    slide_conditions_body: '해당하는 것을 모두 선택하세요. AI 인사이트가 개인화됩니다. 나중에 언제든지 변경할 수 있습니다.',
    slide_ready_title: '준비 완료!',
    slide_ready_body: '첫 번째 체크인은 약 30초면 완료됩니다. 더 많이 기록할수록 Symply가 내 몸을 더 잘 이해합니다.',
    slide_ready_tip_ios: '💡 팁: Safari에서 공유(Share) → 홈 화면에 추가를 눌러 앱처럼 설치하세요.',
    slide_ready_tip_android: '💡 팁: Chrome에서 메뉴(⋮) → 홈 화면에 추가를 눌러 앱처럼 설치하세요.',
    slide_ready_tip_other: '💡 팁: 브라우저 메뉴에서 홈 화면에 추가하면 더 빠르게 접근할 수 있습니다.',
    how_works: [
      { icon: '⏱️', title: '30초 체크인', body: '통증, 피로, 수면, 트리거를 30초 이내에 기록하세요.' },
      { icon: '🤖', title: 'AI 패턴 발견', body: 'Claude AI가 데이터를 분석해 개인화된 인사이트를 제공합니다.' },
      { icon: '📄', title: '진료 준비 보고서', body: '의사가 3분 안에 읽을 수 있는 임상 PDF를 자동 생성합니다.' },
    ],
    preview_demo_badge: '🎁 예시 화면 (실제 내 기록 아님)',
    preview_chart_caption: '일별 통증·피로 점수 (0~10, 최근 10일)',
    preview_legend_pain: '통증',
    preview_legend_fatigue: '피로',
    preview_summary: '생리 주기, 수면, 식이 트리거와 관련된 명확한 증상 패턴이 나타납니다.',
    preview_pattern1_title: '생리 2~3일 전 통증이 급증',
    preview_pattern1_desc: '생리 시작 2~3일 전 통증 점수가 평균 40% 높아집니다. 자궁내막증·PCOS에서 흔히 나타나는 월경 전 증상 패턴과 일치합니다.',
    preview_pattern2_title: '중간 활동량인 날이 가장 좋은 날',
    preview_pattern2_desc: '중간 활동량인 날은 통증과 피로의 합산 점수가 가장 낮습니다.',
    preview_doctor_label: '의사에게 전달할 내용',
    preview_doctor_point1: '생리 2~3일 전 통증이 일관되게 급증합니다 — 월경 전 플레어가 의심됩니다',
    preview_doctor_point2: '중간 활동량인 날에 전반적인 증상 조절이 가장 잘 됩니다',
  },

  upgrade: {
    title: 'Pro로 업그레이드',
    subtitle: '{feature}는 Pro 기능입니다. 커피 한 잔 가격으로 모든 Pro 기능을 사용하세요.',
    feature_ai: 'AI 패턴 분석',
    feature_report: '진료 준비 보고서 PDF',
    features: [
      { icon: '🤖', text: 'AI 패턴 분석 — 개인화된 인사이트' },
      { icon: '📄', text: '진료 준비 보고서 PDF — 임상 요약' },
      { icon: '📊', text: '무제한 기록 & 고급 차트' },
    ],
    price: '$3.99',
    per_month: '/ 월',
    annual: '또는 $29.99/년 · 언제든 취소 가능',
    cta: '✨ Pro 시작 — $3.99/월',
    loading: '⏳ 결제 페이지로 이동 중...',
    later: '나중에',
  },

  guide: {
    title: 'Symply 용어 설명',
    intro: '통증·피로 척도, 질환, 트리거 등 앱에서 사용하는 용어를 풀어드립니다. "고포드맵 식사"처럼 낯선 용어가 나오면 여기서 찾아보세요. 화면별 사용법도 아래에 간단히 정리했습니다.',
    pages_title: '화면별 안내',
    pages: [
      {
        icon: '🏠',
        title: '홈 — 오늘의 체크인',
        desc: '약 30초 안에 오늘의 증상을 기록하세요. 통증, 피로도, 기분, 수면 시간, 활동량, 오늘의 영향 요인을 입력할 수 있습니다. 통증과 피로도 척도에는 실제 비유 설명이 포함되어 있어 날마다 일관된 기록이 가능합니다.',
      },
      {
        icon: '📅',
        title: '기록 — 나의 기록 보기',
        desc: '색상 코드 달력 히트맵으로 지난 기록을 확인하세요. 날짜를 탭하면 해당 날의 상세 기록을 볼 수 있습니다.',
      },
      {
        icon: '🌸',
        title: '사이클 — 생리주기 추적',
        desc: '성별을 여성으로 설정한 사용자라면 누구나 사용할 수 있습니다. 생리 여부, 출혈량, 주기 관련 증상을 기록하세요. 이 데이터는 AI 분석에 자동으로 반영됩니다. 루푸스, 섬유근통, 류마티스 관절염, 과민성 대장 증후군 등 많은 만성질환이 호르몬 주기의 영향을 받습니다.',
      },
      {
        icon: '📊',
        title: '인사이트 — 트렌드 및 AI 분석',
        desc: '통증, 피로도, 수면, 기분의 트렌드 차트를 확인하세요. Pro 사용자는 AI 기반 패턴 분석을 이용할 수 있습니다. 무료 사용자는 월 5회까지 AI 분석을 이용할 수 있습니다.',
      },
      {
        icon: '📄',
        title: '보고서 — 진료 준비 PDF (Pro)',
        desc: '수개월의 기록을 바탕으로 1~2페이지 분량의 임상 요약 PDF를 자동 생성합니다. 의사가 3분 안에 읽을 수 있는 형식으로 설계되었습니다. 진료 전날 생성하는 것을 권장합니다.',
      },
      {
        icon: '⚙️',
        title: '설정',
        desc: '성별, 만성질환, 복약, 테마, 언어, 일일 알림, 구독을 관리할 수 있습니다.',
      },
    ],
    terms_title: '주요 용어',
    evidence_title: '트리거 근거 참고자료',
    evidence_intro: '각 트리거가 질환별로 얼마나 확립된 연구 근거를 갖고 있는지 보여드립니다. ★★★ = 강한 근거, ★★ = 중간 근거, ★ = 약하거나 예비 단계 근거. 이는 발표된 연구의 확립 정도를 반영할 뿐, 개인에게 실제로 얼마나 영향을 미치는지와는 다릅니다.',
    evidence_note_functional_only: '기능성 증상(예: 팽만감)에 대한 근거이며, 실제 측정된 질병 활성도에 대한 근거는 아닙니다.',
    evidence_note_reverse_direction: '일부 연구는 오히려 반대 방향(악화가 아닌 완화)을 시사합니다.',
    evidence_note_self_report_only: '주로 환자 자기보고에 근거하며, 대조 연구에서는 중립적이거나 엇갈린 결과를 보입니다.',
    terms: [
      {
        icon: '📏',
        term: '통증 앵커 척도',
        desc: '통증은 0~10 척도로 기록합니다. 주요 레벨의 설명:\n\n0 (없음) — 통증 없음. 완전히 정상.\n2 (약함) — 신경 쓰이지만 하던 일을 계속할 수 있음.\n4 (보통) — 집중하기 어렵지만 참을 수 있음. 심한 두통 수준.\n6 (심함) — 하던 일을 멈춰야 함. 심한 치통 수준.\n8 (극심함) — 말하기도 힘듦. 신장결석·심한 골절 수준.\n10 (최악) — 출산 진통 수준. 의식이 흐려질 정도.\n\n1, 3, 5, 7, 9 등 중간 값은 두 앵커 사이에 해당할 때 선택할 수 있습니다.',
      },
      {
        icon: '😴',
        term: '피로 앵커 척도',
        desc: '피로도는 0~10 척도로 기록합니다. 루푸스·MS 환자를 위해 개발된 FSS(피로 심각도 척도) 개념을 참고하여 설계되었습니다.\n\n0 (활기참) — 완전히 활기참. 피로가 일상 활동에 전혀 영향 없음.\n2 (약한 피로) — 약간 피곤하지만 모든 일상 활동 수행 가능.\n4 (보통 피로) — 피로로 인해 일부 활동이 제한됨.\n6 (심한 피로) — 피로가 대부분의 활동을 방해함. 직장·가정생활 영향.\n8 (극심한 피로) — 대부분의 활동 불가능. 기본적인 일도 힘듦.\n10 (완전탈진) — 침대에서 전혀 못 일어남. 완전히 일상이 마비됨.\n\n두 앵커 사이에 해당하면 중간 값을 선택할 수 있습니다.',
      },
      {
        icon: '🔍',
        term: '오늘의 영향 요인',
        desc: '증상에 영향을 줄 수 있는 요소들을 세 가지 근거 기반 카테고리(음식 및 음료, 생활습관, 환경)로 정리했습니다. 만성질환 연구를 바탕으로 선별된 항목입니다.\n\n설정에서 대표 질환을 지정하면, 그 질환과 가장 관련 있는 요인이 먼저 표시되고 별표(★)로 표시됩니다. 근거 강도는 질환마다 다릅니다 — 예를 들어 고포드맵 식이는 IBS에서 확립된 트리거이지만, 자궁내막증의 식이 트리거는 근거가 상대적으로 약합니다. Symply는 모든 요인을 같은 확신 수준으로 다루지 않고 이 차이를 그대로 반영합니다.',
      },
      {
        icon: '🥗',
        term: '고포드맵(FODMAP) 식사',
        desc: 'FODMAP은 장에서 잘 흡수되지 않고 발효되어 가스·팽만감·복통을 유발할 수 있는 특정 탄수화물류(마늘, 양파, 밀, 일부 과일·유제품 등)를 뜻합니다. IBS(과민성 대장 증후군)에서 특히 확립된 트리거입니다.',
      },
      {
        icon: '🍚',
        term: '고혈당지수(High GI) 식사',
        desc: '혈당지수(GI)가 높아 혈당을 빠르게 올리는 음식(흰쌀, 흰빵, 단 음료 등)입니다. 혈당 급등락은 염증 반응이나 PCOS의 인슐린 저항성과 관련된 증상에 영향을 줄 수 있습니다.',
      },
      {
        icon: '🔄',
        term: 'PCOS (다낭성 난소 증후군)',
        desc: '난소의 호르몬 불균형으로 인해 불규칙한 생리, 안드로겐 과다, 대사 변화 등이 나타나는 질환입니다.',
      },
      {
        icon: '🌸',
        term: '자궁내막증',
        desc: '자궁 내막과 유사한 조직이 자궁 밖에서 자라는 질환으로, 만성 통증과 염증을 유발합니다.',
      },
      {
        icon: '💜',
        term: '섬유근통 (Fibromyalgia)',
        desc: '전신에 광범위한 근골격계 통증, 피로, 수면 장애, 인지 어려움을 일으키는 만성 질환입니다. 스트레스, 수면, 활동량에 의해 증상이 오락가락하는 것이 특징입니다.',
      },
      {
        icon: '🫀',
        term: '루푸스 (SLE)',
        desc: '면역계가 정상 조직을 공격하는 전신성 자가면역질환입니다. 피부, 관절, 신장 등에 영향을 미치며 플레어와 호전이 반복됩니다.',
      },
      {
        icon: '🦴',
        term: '류마티스 관절염 (RA)',
        desc: '자가면역 반응으로 관절 염증, 통증, 부기가 지속적으로 나타나는 질환입니다. 시간이 지나면 다른 신체 기관에도 영향을 줄 수 있습니다.',
      },
      {
        icon: '🪲',
        term: '크론병',
        desc: '소화관 어느 부위든 염증을 일으킬 수 있는 염증성 장질환(IBD)의 일종입니다. 복통, 설사, 피로, 체중 감소가 주요 증상입니다.',
      },
      {
        icon: '⚡',
        term: '과민성 대장 증후군 (IBS)',
        desc: '복통, 팽만감, 배변 변화를 일으키는 기능성 장 장애입니다. 호르몬 주기, 스트레스, 식습관이 증상에 영향을 줍니다.',
      },
      {
        icon: '😴',
        term: '만성 피로 증후군 (ME/CFS)',
        desc: '휴식으로 회복되지 않는 극심한 피로, 인지 장애(뇌안개), 활동 후 증상 악화를 특징으로 하는 만성 질환입니다.',
      },
      {
        icon: '⚡',
        term: '플레어 (Flare)',
        desc: '만성질환 증상이 갑자기 심해지는 기간입니다. 꾸준한 데이터 기록을 통해 플레어를 유발하거나 예고하는 패턴을 파악할 수 있습니다. (예측 알림 기능은 추후 업데이트 예정입니다.)',
      },
    ],
  },

  mood_labels: {
    1: '매우 나쁨',
    2: '나쁨',
    3: '보통',
    4: '좋음',
    5: '최고',
  },

  common: {
    loading: '불러오는 중…',
    error: '오류가 발생했습니다.',
    days: '일',
    hours: '시간',
  },
}
