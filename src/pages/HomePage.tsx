      ...
      <Card style={{ marginBottom: '12px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <button onClick={() => setSleep(s => Math.max(0, s - 0.5))}
              style={{ width: '28px', height: '28px', borderRadius: '50%', border: '1px solid var(--color-border)', background: 'var(--color-surface-2)', cursor: 'pointer', fontSize: '1rem', color: 'var(--color-text)' }}>−</button>
            <span style={{ fontWeight: 700, fontSize: '1.1rem', minWidth: '48px', textAlign: 'center', color: 'var(--color-text)' }}>
              {sleep}h
            </span>
            <button onClick={() => setSleep(s => Math.min(24, s + 0.5))}
              style={{ width: '28px', height: '28px', borderRadius: '50%', border: '1px solid var(--color-border)', background: 'var(--color-surface-2)', cursor: 'pointer', fontSize: '1rem', color: 'var(--color-text)' }}>+</button>
          </div>
        </div>
      </Card>
          ...
          {TRIGGERS.map(({ key, label, emoji }) => (
            <button
              ...
              style={{
                ...
                cursor: 'pointer',
                fontSize: '0.82rem',
                fontWeight: triggers[key] ? 600 : 400,
